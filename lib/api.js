...
export const modules = `
  ...
  _type == 'productsGrid' => {
    ...
    productsList[]->{
      ...
      comparePrice,
      "slug": slug.current,
      "photo": galleryPhotos[0].photos[0]{
        ${imageMeta}
      }
    }
  },
  ...
  _type == 'recipesList' => {
    ...
    subtitle,
    maxNumber,
    "recipes": *[_type == "recipePage"] | order(_createdAt asc)[]{
      title,
      image {
        ${imageMeta}
      },
  ...
  const query = `
    {
      "page": *[_type == "recipePage" && slug.current == "${slug}"] | order(_updatedAt desc)[0]{
        title,
        instructions,
        "ingredientProducts": ingredients.ingredientProducts[wasDeleted != true && isDraft != true]-> {inStock, productID, title, "slug": slug.current},
